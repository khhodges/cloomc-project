...
-   **Canonical Boot Memory Layout**: Boot Namespace starts at address 0x0000. Slot 0=Boot.NS→CR15 (zero perms, Inform), Slot 1=Thread→CR8, Slot 2=C-List→CR6 (E only), Slot 3=Boot.Abstraction (CALL entry, E only, CR7 at offset 0), Slot 4=Boot.CLOOMC, Slot 5+=remaining abstractions. CR5 is NULL at boot. Hardwired GT for CR15: 0x00000000. All boot CRs pass through mLoad (M-elevation bypasses permission but not bounds/version/seal). Boot phases: IDLE→FAULT_RST→LOAD_NS→INIT_THRD(CR8)→INIT_CLIST(CR6)→LOAD_NUC(CALL)→COMPLETE.
