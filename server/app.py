...
- `church_ti60_f225.peri.xml` — Periphery / Interface Design (PLL, UART, LEDs, button)
- `church_ti60_f225.sdc`      — Timing constraints (50 MHz clock)
- `ti60_f225.isf`             — Pin constraints (Interface Setup File)
- `setup_ti60_peri.py`        — Re-generates peri.xml via Efinity DesignAPI (see below)
- `BUILD.md`                  — This file

## Quick Start — Open in Efinity IDE

1. Extract this zip to a folder (e.g. `~/church_ti60/`)
2. Open Efinity IDE, then open the project:
   ```
   File → Open Project → church_ti60_f225.xml
   ```
3. **If Efinity warns "peri.xml corrupted"**, run the setup script first (see below)
4. Run **Synthesis → Place & Route → Generate Bitstream**
5. Flash via **Efinity Programmer** (JTAG)

## If peri.xml Is Reported as Corrupted

The periphery file requires the Efinity DesignAPI to be regenerated with the correct
PLL settings (25 MHz crystal at B2 → PLL_TL0 M=4,N=1,O=2 → 50 MHz GCLK).

Run the included setup script **using Efinity's own Python** from the package folder:
...
This regenerates `church_ti60_f225.peri.xml` correctly and prints `SUCCESS`.
Then open the project in Efinity IDE normally.
