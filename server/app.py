...
HARDWARE_PROFILES = {
    ...
    "tang-nano-9k": {
        "label": "Sipeed Tang Nano 9K (IoT profile)",
        "totalRamWords": 2048,
        "addressBits": 13,
        "addressRange": "0x0000_0000 – 0x0000_1FFF (8 KB byte-addressable)",
        "notes": "Gowin GW1NR-9C — IoT profile (reduced ISA) to fit 8,640 LUT4s",
    },
    "ti60-f225": {
...
_ALLOWED_BUILD_FILES = {
    ...
    "church_tang_nano_9k.v":   "text/plain",
    "church_tang_nano_9k.il":  "text/plain",
}
...
BUILD_MD_9K = """# Church Machine — Tang Nano 9K Build Package (IoT Profile)

## What's Inside

- `church_tang_nano_9k.il` — Amaranth RTLIL (always included — the authoritative design)
- `church_tang_nano_9k.v` — Synthesisable Verilog (may be absent if server synthesis timed out)
- `church_tang_nano_9k.json` — Yosys synthesis netlist (may be absent — regenerated by flash.sh)
- `tang_nano_9k.cst` — Pin constraints for GW1NR-LV9QN88PC6/I5
- `Makefile` — Build automation (pnr, pack, prog targets)
- `flash.sh` — One-command build + flash script (auto-runs Yosys if JSON is missing)
- `bridge.sh` — Connect the flashed board to the Church Machine IDE
- `local_bridge.py` — Serial bridge server (used by bridge.sh)
...
> **IoT Profile:** GC, Lambda, Change, Switch, ELoadCall, and XLoadLambda instructions
> are omitted to reduce LUT count below the Tang Nano 9K's 8,640 LUT4 limit.
...
| LED  | Pin | Signal    | Expected after flash          |
|------|-----|-----------|-------------------------------|
| led0 | 10  | Boot/Run  | Solid ON — boot complete      |
| led1 | 11  | Halt      | Blinking — core halted        |
| led2 | 13  | Fault     | OFF — no capability fault     |
| led3 | 14  | Heartbeat | Blinking ~1 Hz — clock alive  |
...
| 1 solid + 0 blinking | Clock not running | Bad bitstream — rebuild from IDE |
| 2 blinking + 0 solid | Unexpected state | Report with serial log |
| 3 LEDs correct but serial blank | Wrong serial port | Try `/dev/ttyUSB1` instead of `/dev/ttyUSB0` |
| 3 LEDs + serial OK but no call-home | Bridge not running | Run `./bridge.sh --ide=URL`; verify URL matches your IDE |

## Device

- **FPGA**: Gowin GW1NR-LV9QN88PC6/I5 (nextpnr device: GW1N-9C)
- **Board**: Sipeed Tang Nano 9K
- **ISA**: IoT profile — reduced instruction set (no GC, Lambda, Change, Switch, ELoadCall, XLoadLambda)
- **Clock**: 27 MHz crystal (pin 52)
- **UART**: 115200 baud via CH340 USB bridge (pin 18 TX, pin 17 RX)
- **LEDs**: 4 used (pins 10, 11, 13, 14) — active-low
"""
...
### Step 1a — Open in Efinity IDE

Extract the zip anywhere, then:

```
File → Open Project → church_ti60_f225.xml
```

### Step 1b — Configure the PLL (one-time, required for 50 MHz operation)

The bundled `church_ti60_f225.peri.xml` has GPIO pins but **no PLL configured**.
Run this once from inside your Efinity project folder to add PLL_TL0 (25 MHz → 50 MHz):
...
After running, open `church_ti60_f225.peri.xml` in **Interface Designer** to verify the
PLL appears, then edit `church_ti60_f225.sdc`: comment out the Phase A line and
uncomment the three Phase B lines (see comments inside the SDC file).

Without this step the core runs at 25 MHz (UART baud will be half speed) but the
bitstream is otherwise functional.
...
def _fpga_paths(board):
    ...
    if is_ti60:
        ...
        paths = {
            "rtlil":   os.path.join(build_dir, "church_tang_nano_9k.il"),
            "verilog": os.path.join(build_dir, "church_tang_nano_9k.v"),
            "json":    os.path.join(build_dir, "church_tang_nano_9k.json"),
            "cst":     os.path.join(hw_dir,    "tang_nano_9k.cst"),
            "makefile":os.path.join(hw_dir,    "Makefile"),
        }
        zip_name = "church-nano-9k-package.zip"
        build_md = BUILD_MD_9K
