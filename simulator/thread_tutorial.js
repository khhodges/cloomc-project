...
<p>GC uses the <strong>G bit</strong> (Word 3 of each GT) as the mark bit. The root set has two parts: the live GTs in <strong>Zone \u2460 (CR0\u2013CR11)</strong>, and the <strong>E-GT word in every live CALL frame</strong> on the stack (frame word 0 of each 2-word SZ=1 frame, walked from STO upward to word 212). <strong>CR5 is not traced</strong> \u2014 it is a region-coverage token for Zone \u2463 as a whole, not a reference to any particular heap object. From the root set the collector marks all reachable heap GTs transitively, sweeps unreachable objects, then <strong>compacts</strong> the live set toward heap base (word 17). After compaction, <strong>DR5 is reset</strong> to the offset of the word immediately after the compacted live set \u2014 without this reset, DR5 would point into the middle of valid data and corrupt the next allocation. GC reads but does not modify stack frames or Zone \u2460 GTs; the only Data Register it writes is DR5. Collection fires automatically on RETURN; the <strong>Run\u202fGC</strong> button in the simulator provides a manual trigger.</p></div>`
            },
            {
...
<div class="sr-sec-item"><span class="sr-sec-num">7</span><strong>Heap allocation &amp; GC.</strong> Objects are written into the heap via DWRITE using CR9. When freespace is exhausted a <code>FAULT [HEAP_FULL]</code> fires. GC is triggered automatically by RETURN \u2014 only the Thread GT (thread lump) is the GC root; objects unreachable from that root are reclaimed and the live set is compacted in place without moving the LIFO stack or the GT zone. The simulator\u2019s <strong>Run GC</strong> button provides a manual trigger for interactive demonstration only.</div>
</div>
