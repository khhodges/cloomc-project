...
<p>Every time mLoad executes it <strong>writes the loaded GT back into the corresponding GT-zone word</strong> (<code>(lumpSize\u221212)+N</code> for CR_N). This guarantees the lump\u2019s GT zone always mirrors the live CR registers. When CHANGE suspends a thread, DR0\u2013DR15 are saved into the thread image along with the <strong>cursor register</strong> (one 32-bit word encoding both NIA and STO), CR14 (code register), and CR15 (namespace root). CR12 (thread stack) and CR13 (interrupt handler) are system-wide and are <em>not</em> saved \u2014 they remain constant across thread switches. Because mLoad already kept the GT zone in sync during execution, no separate save step is needed for CR0\u2013CR11.</p></div>`
            },
            {
