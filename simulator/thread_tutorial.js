...
<p>Every time mLoad executes it <strong>writes the loaded GT back into the corresponding GT-zone word</strong> (<code>(lumpSize\u221212)+N</code> for CR_N). This guarantees the lump\u2019s GT zone always mirrors the live CR registers. When CHANGE suspends a thread, DR0\u2013DR15 are saved into the thread image along with the <strong>cursor register</strong> (one 32-bit word encoding both NIA and STO), CR14 (code register), and CR15 (namespace root). CR12 (data fault handler) and CR13 (interrupt handler) are system-wide and are <em>not</em> saved \u2014 they remain constant across thread switches. Because mLoad already kept the GT zone in sync during execution, no separate save step is needed for CR0\u2013CR11.</p></div>`
            },
            {
...
<div class="sr-key-concept"><div class="sr-concept-title">CR12 \u2014 Data Fault Handler (Privileged, System-Wide)</div>
<p>Boot step B:02 (INIT_THRD) loads <strong>one</strong> register from NS Slot 1:</p>
<ul>
<li><strong>CR12 \u2014 Data Fault Handler</strong> (Inform-type, zero perms, Priv zone CR12\u2013CR15). Loaded from NS Slot 1 via mLoad at B:02. CR12 holds the capability invoked by hardware on a data fault. Its <code>word0_location</code> encodes the lump base from which stack bounds (<code>sp_max</code>, <code>sp_min</code>) are derived using the <code>sw</code> field in the thread header. The actual stack position is tracked by the <strong>cursor register</strong> (hardware-only 32-bit word: NIA[30:16] | STO[15:0]). CR12 is system-wide and is <em>not</em> saved or restored by CHANGE \u2014 it remains constant across all thread switches. CR13 (interrupt handler) is likewise system-wide and unchanged by CHANGE. CR14 (code register) and CR15 (namespace root) are per-thread and are saved and restored on every CHANGE.</li>
...
<div class="sr-sec-item"><span class="sr-sec-num">1</span><strong>Boot \u2014 INIT_THRD (B:02).</strong> <code>sim._bootStep()</code> loads NS Slot 1 into <strong>CR12</strong> (data fault handler GT, zero perms, Inform-type) via mLoad. CR12 encodes the lump base and total size; the stack region and heap bounds are derived from this metadata by the hardware. The lump is now the active thread context; CR0\u201311 hold the initial capability set. CR8 is not touched at boot and is available for programmer use.</div>
...
<div class="sr-sec-item"><span class="sr-sec-num">5</span><strong>CHANGE \u2014 Suspend &amp; Resume.</strong> CHANGE indexes the invoking thread\u2019s c-list at the given offset (CRd[idx]) to obtain an E-perm Thread Abstraction GT for the target thread. CR8 is <em>not</em> a CHANGE operand and is not touched by CHANGE. The outgoing thread\u2019s context is saved in two parts: (a) <strong>the GT zone (CR0\u2013CR11) is already persisted</strong> \u2014 mLoad wrote every GT back to the lump in real time, so no additional save is needed; (b) CHANGE <strong>explicitly saves</strong> the remaining per-thread live state: <strong>DR0\u2013DR15</strong>, the <strong>cursor register</strong> (one 32-bit word encoding both NIA and STO), <strong>CR14</strong> (code register), and <strong>CR15</strong> (namespace root). CR12 (data fault handler) and CR13 (interrupt handler) are system-wide and are <em>never</em> saved or restored \u2014 they remain constant across all thread switches. NIA and STO are never written to heap memory \u2014 they travel only through the cursor register and the CHANGE save path. The NS slot is updated only if lump metadata has changed.</div>
<div class="sr-sec-item"><span class="sr-sec-num">6</span><strong>Resume.</strong> CHANGE restores the incoming thread\u2019s saved per-thread context: DR0\u2013DR15, the cursor register (restoring both NIA and STO atomically), CR14 (code register), and CR15 (namespace root) are all reloaded from the saved state. The GT zone (CR0\u2013CR11) of the incoming thread is already in its lump and is never explicitly restored \u2014 the GT zone is live at all times. CR12 (data fault handler) and CR13 (interrupt handler) are system-wide and are <em>not</em> changed.</div>
